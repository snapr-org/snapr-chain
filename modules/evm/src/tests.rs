...
fn create_reverts_with_message() {
	...
	new_test_ext().execute_with(|| {
		let result =
			Runner::<Test>::create(trillian(), contract, 0, 12_000_000, 12_000_000, <Test as Config>::config()).unwrap();
...
fn call_reverts_with_message() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_eq!(balance(trillian()), alice_balance);
...
fn should_deploy_payable_contract() {
	...
	new_test_ext().execute_with(|| {
		...
		let result =
			Runner::<Test>::create(trillian(), contract, amount, 100000, 100000, <Test as Config>::config()).unwrap();
		let contract_address = result.address;
		...
		assert_eq!(balance(trillian()), alice_balance - amount);
...
fn should_transfer_from_contract() {
	...
	new_test_ext().execute_with(|| {
		...
		let result = Runner::<Test>::create(trillian(), contract, 0, 10000000, 10000000, <Test as Config>::config())
		...
		assert_eq!(balance(trillian()), alice_balance - 1 * amount);
		...
		assert_eq!(balance(trillian()), alice_balance - 2 * amount);
		...
		assert_eq!(balance(trillian()), alice_balance - 3 * amount);
	})
}
...
fn deploy_factory() {
	...
	new_test_ext().execute_with(|| {
		let result = Runner::<Test>::create(trillian(), contract, 0, 2_000_000, 5000, <Test as Config>::config()).unwrap();
		assert_eq!(result.exit_reason, ExitReason::Succeed(ExitSucceed::Returned));
...
fn should_transfer_maintainer() {
	...
	new_test_ext().execute_with(|| {
		...
		let alice_account_id = <Test as Config>::AddressMapping::get_account_id(&trillian());
		let bob_account_id = <Test as Config>::AddressMapping::get_account_id(&ford());
		assert_eq!(balance(ford()), INITIAL_BALANCE);
		// transfer_maintainer
		...
		let event = Event::evm_mod(crate::Event::TransferredMaintainer(result.address, ford()));
		assert!(System::events().iter().any(|record| record.event == event));
		assert_eq!(balance(ford()), INITIAL_BALANCE);

		assert_noop!(
			EVM::transfer_maintainer(Origin::signed(bob_account_id.clone()), H160::default(), trillian()),
			Error::<Test>::ContractNotFound
		);

		assert_noop!(
			EVM::transfer_maintainer(Origin::signed(alice_account_id.clone()), result.address, ford()),
...
fn should_deploy() {
	...
	new_test_ext().execute_with(|| {
		...
		// create contract
		let result = Runner::<Test>::create(trillian(), contract, 0, 21_000_000, 21_000_000, <Test as Config>::config()).unwrap();
		let contract_address = result.address;
		...
		assert_eq!(balance(trillian()), INITIAL_BALANCE - DeploymentFee::get() - ((NewContractExtraBytes::get() + code_size) as u64 * StorageDepositPerByte::get()));
...
fn should_enable_contract_development() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			Accounts::<Test>::get(trillian()).unwrap().developer_deposit,
			Some(DeveloperDeposit::get())
		);
		assert_eq!(balance(trillian()), INITIAL_BALANCE - DeveloperDeposit::get());
	});
}
...
fn should_disable_contract_development() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(balance(trillian()), INITIAL_BALANCE);
...
fn storage_limit_should_work() {
	...
	new_test_ext().execute_with(|| {
		let result =
			Runner::<Test>::create(trillian(), contract.clone(), 0, 200_000, 1000, <Test as Config>::config()).unwrap();
		assert_eq!(result.exit_reason, ExitReason::Succeed(ExitSucceed::Returned));
